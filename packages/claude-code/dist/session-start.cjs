...
function configWarning(path, reason) {
  return { file: "config", path, reason };
}
function exceptionsWarning(path, reason) {
  return { file: "exceptions", path, reason };
}
function inspectConfig(raw, path) {
  let data;
  try {
    data = JSON.parse(raw);
  } catch {
...
function inspectExceptions(raw, path) {
  let data;
  try {
    data = JSON.parse(raw);
  } catch {
  ...
  return ExceptionsFileSchema.safeParse(data).success ? null : exceptionsWarning(path, "validation");
}
async function getConfigurationWarnings(configPath, logger2) {
  const warnings = [];
  ...
  const config = await loadConfig(resolvedConfigPath, logger2);
...
function formatConfigurationWarnings(warnings, branding) {
  ...
  for (const warning of warnings) {
    if (warning.file === "config") {
      ...
      lines.push(`- ${formatPath(warning.path)} ${reason}. ${branding.short_name} is using default settings until this file is fixed.`);
    } else if (warning.reason === "parse") {
      lines.push(`- ${formatPath(warning.path)} is not valid JSON. Exception rules are not being applied until this file is fixed.`);
    } else {
      lines.push(`- ${formatPath(warning.path)} has the wrong shape. Expected ${JSON.stringify({ rules: [] })}. Exception rules are not being applied until this file is fixed.`);
    }
  }
  return lines.join("\n");
}
...
async function runSessionStartScan(context) {
  ...
  if (plugins.length === 0) {
    logger2.debug("Session plugin scan completed", {
      totalPlugins: 0,
...
function spawnModelDownloadWorker(args) {
  ...
  try {
    ...
    logger2.warn("Failed to spawn model download worker", { error: String(err) });
    return false;
  }
}
...
async function main() {
  ...
  const warningMessage = formatConfigurationWarnings(
    await getConfigurationWarnings(void 0, logger),
    branding
  );
