...
- wire the skill-upload worker into the OpenClaw connector so queued unknown skills are uploaded for analysis

### Patch Changes

- Fix OpenClaw Layer 2 E2E auth in CI by bumping the pinned OpenClaw image to 2026.7.1 and keeping the google-vertex provider on Vertex ADC. OpenClaw 2026.3.28's auth gate only accepted an API key or an on-disk credential file and never consulted the GCE metadata server, so Vertex ADC could not authenticate it headless; 2026.7.1's rewritten vertex-adc module delegates to google-auth-library, which resolves credentials via the full ADC chain (including the metadata server / attached service account). Also surfaces the gateway's container logs when an E2E chat request fails, so the underlying error is no longer hidden behind a generic 500.
