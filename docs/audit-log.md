...
| `pi_checks` | `{ risk: number, model_id: string, content_name: string }` | Prompt-injection (PI) ML model results, gated by [`pi_check.enabled`](user-guide.md#pi_check). `risk` is the model's score in `[0, 1]`. `model_id` is the model directory basename (e.g. `"pi-model"`). `content_name` labels what was scanned — currently always `"WebFetch:<url>"`, since the model runs only on content fetched for `WebFetch` during PreToolUse. Emitted whenever the model is invoked and produces a result. PostToolUse output scanning is heuristic-only and never contributes `pi_checks`; Tier 1 heuristic prompt-injection rules surface separately under `heuristics`. |
